...
def main(config):
    ...
    if False: # create data sample of obs jet and regular jet
        ...
        np.save(f'/srv/beegfs/scratch/groups/rodem/pileup_diffusion/data/data/obs_jet{name}_size_{size}{date_str}.npy', eval_ctxt)

        # Write dictionary to json file
        np.save(f'/srv/beegfs/scratch/groups/rodem/pileup_diffusion/data/data/top_jet_size_{size}{date_str}.npy', eval_truth)
        ...
        if not os.path.isfile(truth_name):
        ...
        file_name = glob(f"/srv/beegfs/scratch/groups/rodem/pileup_diffusion/data/data/obs_jet*{name}*")
